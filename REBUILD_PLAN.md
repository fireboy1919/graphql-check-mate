...
2. Install prerequisites:
   ...
   - Supabase CLI, Podman, and a supabase container.
...
4. Use mise as an environment orchestrator to start Supabase container, backend, and frontend together for day-to-day development.
...
extend type Mutation @scope(to: ["default"]) {
  createCheckboxGroup(input: CreateCheckboxGroupInput!): CheckboxGroup!
    @resolver
  ...
  updateChecklistItem(input: UpdateChecklistItemInput!): ChecklistItem!
    @resolver
    @requiresGroupMembership
  deleteChecklistItem(input: DeleteChecklistItemInput!): Boolean!
    @resolver
    @requiresGroupMembership
}
